...
pub struct HddDetails {
    ...
    pub full_output: String, // Contains the entire smartctl output
}
...
pub struct SmartAttribute {
    ...
    pub raw_value: String, // Changed to String to handle any raw value format
}
...
#[command]
pub fn get_hdd_details(device_path: &str) -> Result<HddDetails, String> {
    log_debug(&format!("Fetching HDD details for device: {}", device_path));
    ...
    // Parsing relevant sections from the output
    ...
    let smart_status = extract_value(
        stdout,
    ...
    log_debug(&format!("Successfully fetched HDD details for device: {}", device_path));
...
#[command]
pub fn get_usb_device_details(device_path: &str) -> Result<UsbDeviceDetails, String> {
    log_debug(&format!("Fetching USB device details for device: {}", device_path));
    ...
    for device in enumerator.scan_devices().map_err(|e| {
        ...
        if let Some(devnode) = device.devnode() {
            if devnode == OsString::from(device_path) {
                ...
                for property in device.properties() {
                    match property.name().to_string_lossy().as_ref() {
                        "CURRENT_TAGS" => details.current_tags = property.value().to_string_lossy().to_string(),
                        "DEVLINKS" => details.devlinks = property.value().to_string_lossy().to_string(),
                        "DEVNAME" => details.devname = property.value().to_string_lossy().to_string(),
                        "DEVPATH" => details.devpath = property.value().to_string_lossy().to_string(),
                        "DEVTYPE" => details.devtype = property.value().to_string_lossy().to_string(),
                        "DISKSEQ" => details.diskseq = property.value().to_string_lossy().to_string(),
                        "ID_BUS" => details.id_bus = property.value().to_string_lossy().to_string(),
                        "ID_FS_BOOT_SYSTEM_ID" => details.id_fs_boot_system_id = property.value().to_string_lossy().to_string(),
                        "ID_FS_DATA_PREPARER_ID" => details.id_fs_data_preparer_id = property.value().to_string_lossy().to_string(),
                        "ID_FS_LABEL" => details.id_fs_label = property.value().to_string_lossy().to_string(),
                        "ID_FS_LABEL_ENC" => details.id_fs_label_enc = property.value().to_string_lossy().to_string(),
                        "ID_FS_TYPE" => details.id_fs_type = property.value().to_string_lossy().to_string(),
                        "ID_FS_USAGE" => details.id_fs_usage = property.value().to_string_lossy().to_string(),
                        "ID_FS_UUID" => details.id_fs_uuid = property.value().to_string_lossy().to_string(),
                        "ID_FS_UUID_ENC" => details.id_fs_uuid_enc = property.value().to_string_lossy().to_string(),
                        "ID_FS_VERSION" => details.id_fs_version = property.value().to_string_lossy().to_string(),
                        "ID_INSTANCE" => details.id_instance = property.value().to_string_lossy().to_string(),
                        "ID_MODEL" => details.id_model = property.value().to_string_lossy().to_string(),
                        "ID_MODEL_ENC" => details.id_model_enc = property.value().to_string_lossy().to_string(),
                        "ID_MODEL_ID" => details.id_model_id = property.value().to_string_lossy().to_string(),
                        "ID_PART_TABLE_TYPE" => details.id_part_table_type = property.value().to_string_lossy().to_string(),
                        "ID_PART_TABLE_UUID" => details.id_part_table_uuid = property.value().to_string_lossy().to_string(),
                        "ID_PATH" => details.id_path = property.value().to_string_lossy().to_string(),
                        "ID_PATH_TAG" => details.id_path_tag = property.value().to_string_lossy().to_string(),
                        "ID_REVISION" => details.id_revision = property.value().to_string_lossy().to_string(),
                        "ID_SERIAL" => details.id_serial = property.value().to_string_lossy().to_string(),
                        "ID_SERIAL_SHORT" => details.id_serial_short = property.value().to_string_lossy().to_string(),
                        "ID_TYPE" => details.id_type = property.value().to_string_lossy().to_string(),
                        "ID_USB_DRIVER" => details.id_usb_driver = property.value().to_string_lossy().to_string(),
                        "ID_USB_INTERFACES" => details.id_usb_interfaces = property.value().to_string_lossy().to_string(),
                        "ID_USB_INTERFACE_NUM" => details.id_usb_interface_num = property.value().to_string_lossy().to_string(),
                        "ID_VENDOR" => details.id_vendor = property.value().to_string_lossy().to_string(),
                        "ID_VENDOR_ENC" => details.id_vendor_enc = property.value().to_string_lossy().to_string(),
                        "ID_VENDOR_ID" => details.id_vendor_id = property.value().to_string_lossy().to_string(),
                        "MAJOR" => details.major = property.value().to_string_lossy().to_string(),
                        "MINOR" => details.minor = property.value().to_string_lossy().to_string(),
                        "SUBSYSTEM" => details.subsystem = property.value().to_string_lossy().to_string(),
                        "TAGS" => details.tags = property.value().to_string_lossy().to_string(),
                        "USEC_INITIALIZED" => details.usec_initialized = property.value().to_string_lossy().to_string(),
                        _ => {},
                    }
                }
    ...
    let err_msg = format!("Device with path {} not found", device_path);
    log_error(&err_msg);
    Err(err_msg)
}
